...
//!      getHeaders (1000/req).
//!   2. Otherwise the public bulk-header CDN — one bounded ranged slice per
//!      tick. Self-healing and needs no external config; recovers on its own
//!      from any large gap within the CDN snapshot, and keeps making progress
//!      even when WoC's tip endpoint is rate-limited (the CDN needs no WoC).
//!   3. WhatsOnChain one-by-one — only for the small remaining gap above the
//!      CDN snapshot (reorg-aware), and for live sync (gap <= 10).
//!
...
pub async fn poll_for_new_blocks(env: &Env) -> Result<()> {
    ...
    // The bulk-CDN catch-up path inserts active rows then flips the tip flag
    // last, so an interrupted tick can leave headers stranded above a stale
    // is_chain_tip row. Cheap: one indexed lookup + a two-statement swap.
    ...
    let chain_info = match client.get_chain_info().await {
        Ok(info) => info,
        Err(e) => {
            // WoC's tip is unreachable — its rate limit is per-IP and Workers
            // egress IPs are shared, so /chain/info can 429 for stretches. The
            // bulk CDN needs no live WoC, so keep closing the historical gap
            // instead of idling the whole tick.
            console_log!("Cron: WoC tip unavailable ({e:?}) — bulk-CDN-only pass");
            if upstream_url.is_none() {
                if let Ok(Some(tip)) = catch_up_from_bulk_cdn(&db, &chain, our_height).await {
                    console_log!("Cron: bulk CDN advanced to {tip} without a live tip");
                }
            }
            return Ok(());
        }
    ...
    if gap > 10 {
        ...
        if let Some(url) = upstream_url.as_deref() {
            ...
            // Self-healing default (no upstream configured): pull the public
            // bulk-header CDN one ranged slice per tick until its snapshot is
            // exhausted, then WoC one-by-one for the small remaining gap to
            // the live tip. Needs no live WoC tip, so it recovers on its own
            // from any large gap within the snapshot.
...
async fn catch_up_via_woc(db: &D1Database, client: &WocClient, our_height: u32, woc_height: u32) {
    ...
    for h in (our_height + 1)..=end {
        match client.get_header_by_height(h).await {
            Ok(header) => {
                if insert_with_parent_backfill(db, client, header).await.is_err() {
                    break;
                }
            }
            Err(e) => {
